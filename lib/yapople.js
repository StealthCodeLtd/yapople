 ...
 * @param {Buffer} data
 */
function onData(timeoutTimer, data) {
  clearTimeout(timeoutTimer);
  var err = null,
    succ = "",
    sData = data.toString();
  if (typeof this.flow === "undefined") {
    // if we have first data chunk from server
    ...
    if (
      ...
      (this._command.cmd === state.LIST &&
        typeof this._command.number === "undefined") ||
      ...
      this._command.cmd === state.USER ||
      this._command.cmd === state.CONNECTING
    ) {
      if (err) {
  ...
  if (typeof this.flow !== "undefined") {
    // for first and all next data chunks
...
Client.prototype.connect = function (options, callback) {
  ...
  if (this.tls) {
    this._socket = tls.connect(options, function () {}.bind(this));
  } else {
    this._socket = net.createConnection(options, function () {}.bind(this));
  }
  const timeoutTimer = setTimeout(() => {
    const err = { message: "Timed out while authenticating with server" };
    this._socket.end();
    callback(err);
    this._queue = [];
  }, options.timeout);
  this._socket.on("data", onData.bind(this, timeoutTimer));
  this._socket.on(
    "error",
    function (err) {
      callback(err);
  ...
  if (this._command.cmd === state.NOOP && this._queue.length) {
    ...
    switch (this._command.cmd) {
      ...
      case state.STAT:
        ...
        break;
      case state.LIST:
        this._write("LIST", this._command.number);
        break;
      case state.RETR:
        this._write("RETR", this._command.number);
        break;
      case state.DELE:
        this._write("DELE", this._command.number);
        break;
      case state.QUIT:
        this._write("QUIT");
        break;
      ...
      case state.TOP:
        this._write(
          "TOP",
          this._command.number + " " + this._command.linesCount
        );
        break;
    }
  }
...
if (parseInt(process.versions.node.split(".")[0]) >= 8) {
  ...
  Client.prototype.retrieveAndDeleteAll = util.promisify(
    Client.prototype.retrieveAndDeleteAll
  );
